...
router.get('/test/:username', async (req, res) => {
    try{
        ...
        if(!user){
            user = await userTest.findOne({email: req.params.username});
            if (!user) {
                console.log("GET /user/"+req.params.username.toString()+" ERROR 422");
                res.status(422).json({message: "User not found"});
                return;
...
router.patch('/test/:id', async (req, res) => {
    ...
    console.log("PATCH /user/"+id);
    try {
        const updUser =  await userTest.findByIdAndUpdate(id, user);
...
router.delete('/test/:id', async (req, res) => {
    ...
    console.log("DELETE /user/"+id);
    try {
        const delUser = await userTest.findByIdAndDelete(id);
        if(!delUser){
            res.status(422).json({message: "User not found"});
            return;
        }
